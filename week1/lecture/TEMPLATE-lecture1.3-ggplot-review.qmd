...
ggplot(penguins, aes(x = bill_length_mm, 
                     ...
                     color = species)) +
  geom_point() +
  scale_color_manual(values = c("blue", "purple", "darkorange"))
```

### Example 3 (mapping color to continuous variable = body mass)
...
ggplot(penguins, aes(x = bill_length_mm, 
                     y = bill_depth_mm)) +
  geom_point(aes(color = species)) + #just want color to apply to the points not the lm
  geom_smooth(method = "lm")
```
...
ggplot(penguins, aes(x = species)) +
  geom_bar(stat = "count") #this is the default stat
```
