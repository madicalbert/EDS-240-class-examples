...
ggplot(lyme_pop, aes(x = year, y = cases_per100k, 
                     group = state)) + #this will group by states
  geom_line() +
  geom_point()
```
...
#adds 175 tick marks on the y axis 
```
...
lyme_pop |> 
  ...
  geom_line() +
  scale_y_continuous(breaks = seq(0, 190, by = 1)) +
  coord_fixed(ratio = 1) #x and y units are the same 
```
...
lyme_pop |> 
  ...
  ggplot(aes(x = year, y = cases_per100k, group = state)) +
  geom_line() +
  coord_fixed(ratio = 1/50) #values larger than 1 have longer y axis, and less than 1 value has shorter y axis then x. 
```
...
lyme_pop |> 
  ...
  ggplot(aes(x = year, 
             ...
             fill = state)) +
  geom_area() +
  scale_x_discrete(expand = c(0, NA)) + #to remove the extra grey space around the filled area
...
lyme_pop |> 
  ...
  geom_area(position = "fill") #change default from stacked to fill will make it proportional 
```
