...
mko_clean %>% 
  #overwrite the month_name column and change characters to factors
  mutate(month_name = factor(month_name, levels = month.name)) %>% #month.name is built into R
  ggplot(aes(x = Temp_bot, fill = month_name)) +
  geom_histogram(position = "identity", alpha = 0.5) #change from default "stacked" and opacity

# still a bit hard to interpret 
```
...
mko_clean |> 
  ...
  ggplot(aes(x = Temp_bot)) + #remove color, redundant and unnecessary 
  geom_histogram() + 
...
mko_clean |> 
  ...
  ggplot(aes(x = Temp_bot, fill = month_name)) +
  #binwidth depends on your research question and what you are visualilzing
...
mko_clean |> 
  filter(month_name %in% c("April", "June", "October")) |> 
  ggplot(aes(x = Temp_bot, fill = month_name)) +
  # smaller binwidth (adjust = 0.5) will give you more variation b/c picking up on all the minute variations in the data, larger binwidth (adjust = 2) with give you less variation, default = 1
...
ggplot(mko_clean, aes(x = Temp_bot, y = month_name, fill = after_stat(x))) + 
  # have to specify where you want the gradient to go with after_stat
  
  geom_density_ridges_gradient(rel_min_height = 0.01, #rel_min_height to shorten tails
                               scale = 3) + # to set the height of the ridges and overlap 
  
  scale_fill_gradientn(colors = c("#2C5374","#849BB4", "#D9E7EC", "#EF8080", "#8B3A3A")) +
  
  #to adjust the order of the y variables, represent order of months
  scale_y_discrete(limits = rev(month.name)) #use rev() to reverse the order!
...
ggplot(mko_clean, aes(x = month_name, y = Temp_bot)) +
  geom_boxplot() +
  scale_x_discrete(limits = rev(month.name)) + #to reorder the months
  coord_flip() # to better visualize the month names 
```
...
ggplot(mko_clean, aes(x = month_name, y = Temp_bot, fill = month_name)) + #color fill the highlight 
  geom_boxplot() +
  scale_x_discrete(limits = rev(month.name)) + 
  gghighlight(month_name == "October") + #to highlight just one variable
  coord_flip() +
...
# add jittered data points ----
 ggplot(penguins, aes(x = species, y = body_mass_g)) +
  geom_boxplot(outlier.shape = NA) + #removes outliers to avoid repeat data 
...
ggplot(mko_clean, aes(x = month_name, y = Temp_bot)) +
  geom_violin() +
  geom_boxplot(width = 0.1,  #width of boxplots, play around w/ numbers 
               color = "gray",
