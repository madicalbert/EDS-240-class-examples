...
con_color_plot +
  scale_color_viridis_c(option = "magma", direction = -1) #direction to flip the scale 
```
...
#same plot as above but written out 
...
ggplot(penguins_ex1, aes(x = bill_length_mm, y = bill_depth_mm, color = my_color)) +
  geom_point(size = 4, alpha = 0.8) +
  scale_color_identity() #need to add this otherwise will assign default colors 
```
